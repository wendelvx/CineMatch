...
export default function Home() {
    ...
    const [selectedGenre, setSelectedGenre] = useState('28'); // 28 é Ação
    ...
    const handleCreateRoom = async () => {
        setLoading(true);
        try {
            const response = await api.post('/rooms', { genreId: selectedGenre });
        ...
        <div className="flex flex-col items-center justify-center min-h-screen p-6 text-center">
            ...
            <div className="w-full max-w-md space-y-6">
                <div className="mb-4 text-left">
                    <label className="text-gray-400 text-sm ml-2">Escolha o Gênero:</label>
                    <select
                        value={selectedGenre}
                        onChange={(e) => setSelectedGenre(e.target.value)}
                        className="w-full mt-1 bg-gray-800 border border-gray-700 text-white p-3 rounded-xl focus:ring-2 focus:ring-red-500 outline-none appearance-none"
                    >
                        {GENRES.map(g => (
